...
let MerchantsService = class MerchantsService {
    constructor(merchantRepo, userRepo) {
        this.merchantRepo = merchantRepo;
        this.userRepo = userRepo;
    }
    async upgradeToBusiness(userId, businessName) {
        const user = await this.userRepo.findOne({ where: { id: userId } });
        if (!user) {
            throw new common_1.NotFoundException('Utilisateur introuvable');
        }
        if (user.accountType === user_entity_1.AccountType.BUSINESS) {
            throw new common_1.BadRequestException('Déjà un compte BUSINESS');
        }
        user.accountType = user_entity_1.AccountType.BUSINESS;
